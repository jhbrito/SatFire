...
datasetPath = r"E:\OneDrive - Instituto Politécnico do Cávado e do Ave\Desktop_backup\Tese\Scripts\Thesis\dataset"
modelsPath = "E:\\OneDrive - Instituto Politécnico do Cávado e do Ave\\Desktop_backup\\Tese\\Scripts\\Tensorflow\\HelloWorlds\\models"
resultsPath = "E:\\OneDrive - Instituto Politécnico do Cávado e do Ave\\Desktop_backup\\Tese\\Scripts\\Tensorflow\\HelloWorlds\\membrane/test/predict"
    ...
    if batch_size > 0:
        while 1:
            ...
            for batchID in range(nBatches):
                images = np.zeros(((batch_size,) + inputSize + (inputChannels,))) # 14 channels
                image_cube_all = np.zeros((inputSize + (inputChannels,))) # 14 channels
...
def unetCustom(pretrained_weights=None, inputSize=(256, 256, 22), numClass=2, do_batch_normalization=False, use_transpose_convolution=False):
    ...
    conv10 = tf.keras.layers.Conv2D(2, 1, activation='softmax', kernel_initializer='he_normal')(withWeather)
...
def saveResults(testSetX, results, resultsPath):
    for i,item in enumerate(results):
        ...
        skimage_io.imsave(os.path.join(resultsPath, os.path.basename(filename) + ".png" ), mask_predict)
...
def main():
    ...
    testGene = testGenerator(testPaths, inputSize=inputSize, inputChannels=1)
    NTest=len(testPaths)
    testSteps = np.ceil(NTest / batchSize)
    results = model.predict(testGene, verbose=1)
    saveResults(testPaths, results, resultsPath)
